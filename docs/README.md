...
| `design/icons/` | The eight theme icons and the sprite, the favicon and social-image sources, the contact sheet, and the generator (`python3 build.py` regenerates everything and `measure.py` reports ink coverage and counter legibility at 15 and 24px). | When a theme is added, renamed or rescoped: redraw the set, not one glyph, and re-run the sheet. |
